...
+ (NSArray *) findNewByCategory:(NSInteger)offset category:(NSString *)category myArray:(NSMutableArray *) myArray{
	PLSqliteDatabase *dataBase = [database setup];
	
	id<PLResultSet> rs;
    NSString *findSql = [NSString stringWithFormat:@"SELECT * FROM bbc where category = %@ ORDER BY voaid desc LIMIT 10 offset %d;",category,offset];
	rs = [dataBase executeQuery:findSql];
//    NSLog(@"sql:%@",findSql);
