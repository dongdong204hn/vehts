...
//	传进来的参数引用+1
	...
	id<PLResultSet> rs;
	NSString *findSql = [NSString stringWithFormat:@"select timing,sentence FROM bbcdetail WHERE voaid = %d order by paraid ", voa._voaid];
...
+ (void)querySQL:(NSMutableArray *)lyricArray 
 lyricCnResultIn:(NSMutableArray *)lyricCnArray 
   ...
   indexResultIn:(NSMutableArray *)indexArray
    ...
    PLSqliteDatabase *dataBase = [database setup];
	id<PLResultSet> rs;
	NSString *findSql = [NSString stringWithFormat:@"select timing,sentence,sentence_cn FROM bbcdetail WHERE voaid = %d order by paraid ", voa._voaid];
	rs = [dataBase executeQuery:findSql];
    int myIndex = 0;
    int time = 0;
	while ([rs next]) {
