...
- (NSInteger)tableView:(UITableView *)tableView //明确cell数目
 numberOfRowsInSection:(NSInteger)section {
    return self.searchFlg == 11 ?[self.contentsArray count] : [_contentsSrArray count]+2;
}
...
{
    if (kIsLandscapeTest) {
        return self.searchFlg == 11 ?(_isiPhone?165.0f:280.0f):( [indexPath row] < [_contentsSrArray count]? (_isiPhone?165.0f:280.0f): (([indexPath row] < [_contentsSrArray count]+1)?(_isiPhone?42.0f:48.0f):1.0f));
    }
    return self.searchFlg == 11 ?(_isiPhone?110.0f:210.0f):( [indexPath row] < [_contentsSrArray count]? (_isiPhone?110.0f:210.0f): (([indexPath row] < [_contentsSrArray count]+1)?(_isiPhone?28.0f:48.0f):1.0f));
}
...
- (void)catchResult:(NSString *) searchWord page:(NSInteger)page{
    ...
    NSString *url = [NSString stringWithFormat:@"http://voa.iyuba.com/voa/searchApi.jsp?key=%@&format=xml&pages=%d&pageNum=10&parentID=%d&fields=all",searchWord,page,_searchFlg];
