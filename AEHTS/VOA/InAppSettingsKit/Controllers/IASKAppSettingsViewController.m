...
- (void)awakeFromNib {
    ...
    //    if ([deviceName isEqualToString:@"ipad"] || [deviceName isEqualToString:@"iPad Simulator"]) {
    //        isiPhone = NO;
    //    }else {
    //        isiPhone = YES;
    //    }
...
//    [[NSBundle mainBundle] loadNibNamed:@"IASKAppSettingsView" owner:self options:nil];
    isiPhone = ![self isPad];
	if (!isiPhone) {
        [[NSBundle mainBundle] loadNibNamed:@"IASKAppSettingsView-iPad" owner:self options:nil];
		self.tableView.separatorStyle = UITableViewCellSeparatorStyleSingleLineEtched;
	}else {
        [[NSBundle mainBundle] loadNibNamed:@"IASKAppSettingsView" owner:self options:nil];
    }
...
//        [cell setBackgroundColor:(isiPhone?[UIColor colorWithRed:0.776 green:0.902 blue:0.973 alpha:1]:[UIColor whiteColor])];/////
//        [cell setBackgroundColor:(isiPhone?[UIColor colorWithRed:0.863f green:0.957 blue:0.827 alpha:1]:[UIColor whiteColor])];/////
        [cell setBackgroundColor:[UIColor colorWithRed:0.863f green:0.957 blue:0.827 alpha:1]];/////
        return cell;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[specifier type] isEqualToString:kIASKPSToggleSwitchSpecifier]) {
        ...
        [targetViewController setHidesBottomBarWhenPushed:YES];//设置推到新界面时无bottomBar
        ...
        if (vcClass) {
			...
			self.navigationController.delegate = nil;
//            [self.navigationController.navigationBar setTintColor:[UIColor colorWithRed:0.77 green:0.07 blue:0 alpha:1]];/////
//            [self.navigationController.navigationBar setTintColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"title.png"]]];
            [vc setHidesBottomBarWhenPushed:YES];//设置推到新界面时无bottomBar
...
{
//    NSLog(@"key :%@",key);
    if ([key isEqualToString:@"DeletePicture"]) {
//        NSLog(@"删除缓存");
        NSString *documentPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
        NSString *userPath = [documentPath stringByAppendingPathComponent:@"ImageCache"];
        //        NSString *path = [[NSBundle mainBundle]  inDirectory:<#(NSString *)#>];
