...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [FlurryAnalytics startSession:@"5SDJBSXK4NDJZ942WDKR"];
...
//            [[NSUserDefaults standardUserDefaults] setObject:@"YES" forKey:@"recordRead"];
            [[NSUserDefaults standardUserDefaults] setFloat:1.2f forKey:@"appVersionBBCF"];
            ...
            if (appVersion < 1.2f) { //新版本的一些新设置等
                [[NSUserDefaults standardUserDefaults] setFloat:1.2f forKey:@"appVersionC"];
...
- (void)modalView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {//点击确定下载为0，取消为1
        ...
        if (alertView.tag == 1){
            ...
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"itms-apps://ax.itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?type=Purple+Software&id=584717854"]];
        }
    }
    [alertView release];
}
...
{
    NSString *url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/phoneToken.jsp?token=%@&appID=584717854",token];
//    NSLog(@"token:%@", token);
